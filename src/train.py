...
def train(args, data_info):
    ...
    user_init_triple_set = data_info[5]
    item_potential_triple_set = data_info[6]
    ...
    for step in range(args.n_epoch):
        ...
        while start < train_data.shape[0]:
            labels = _get_feed_label(
                args, train_data[start : start + args.batch_size, 2]
            )
            ...
            scores, CL_loss = model(
                *_get_feed_data(
                    args,
                    train_data,
                    user_init_triple_set,
                    item_potential_triple_set,
                    user_potential_triple_set,
                    item_origin_triple_set,
                    start,
        ...
        print(
            "Execution time for each sample: %s "
            % ((time.time() - start_time) / len(test_data))
        )
        ctr_info = "epoch %.2d    train auc: %.4f f1: %.4f train f1 eval auc: %.4f eval f1: %.4f    test auc: %.4f test f1: %.4f"

        logging.info(
            ctr_info, step, train_auc, train_f1, eval_auc, eval_f1, test_auc, test_f1
        )
        if args.show_topk:
            topk_eval(
                args,
...
def _init_model(args, data_info):
    ...
    model = FGKAN(args, n_entity, n_relation)
    if args.use_cuda:
        model.cuda()
...
def _get_feed_data(
    ...
    users_init_triple = _get_triple_tensor(
        args, data[start:end, 0], user_init_triple_set
    )
    item_potential_triple = _get_triple_tensor(
        args, data[start:end, 1], item_potential_triple_set
    )
    user_potential_triple = _get_triple_tensor(
        args, data[start:end, 0], user_potential_triple_set
    )
    item_origin_triple = _get_triple_tensor(
        args, data[start:end, 1], item_origin_triple_set
    )
    return (
        items,
        users_init_triple,
        item_potential_triple,
        user_potential_triple,
        item_origin_triple,
    )
