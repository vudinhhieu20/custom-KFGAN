...
class FGKAN(nn.Module):
    def __init__(self, args, n_entity, n_relation):
        ...
        self.attention = nn.Sequential(
            nn.Linear(2 * self.dim * self.n_layer, self.dim, bias=False),
            nn.Sigmoid(),
    ...
    def forward(
        ...
        tmp_h_set_emb = self.entity_emb(user_init_triple_set[0][0])
        tmp_path_emb = self.relation_emb(user_init_triple_set[1][0])
        tmp_t_emb = self.entity_emb(user_init_triple_set[2][0])
        ...
        for i in range(self.n_layer):
            ...
            for k in range(1, i + 1):
                ...
                path_emb = torch.mul(
                    path_emb, self.relation_emb(user_init_triple_set[1][i])
                )
            # [batch_size, triple_set_size, dim]
            ...
            if i != 0:
                # print("user_embeddings Before: ")
                # print(tmp_h_set_emb.shape)
                # print(tmp_path_emb.shape)
                # print(tmp_t_emb.shape)
                tmp_h_set_emb = torch.cat((tmp_h_set_emb, h_set_emb), dim=-1)
                tmp_path_emb = torch.cat((tmp_path_emb, path_emb), dim=-1)
                tmp_t_emb = torch.cat((tmp_t_emb, t_emb), dim=-1)
                # print("user_embeddings After: ")
                # print(tmp_h_set_emb.shape)
                # print(tmp_path_emb.shape)
                # print(tmp_t_emb.shape)

        user_emb_i = self._knowledge_attention(tmp_h_set_emb, tmp_path_emb, tmp_t_emb)
        user_embeddings.append(user_emb_i)
        ...
        tmp_h_set_emb = self.entity_emb(item_potential_triple_set[0][0])
        tmp_path_emb = self.relation_emb(item_potential_triple_set[1][0])
        tmp_t_emb = self.entity_emb(item_potential_triple_set[2][0])
        ...
        for i in range(self.n_layer):
            ...
            if i != 0:
                ...
                tmp_path_emb = torch.cat((tmp_path_emb, path_emb), dim=-1)
                tmp_t_emb = torch.cat((tmp_t_emb, t_emb), dim=-1)
                # print("item_embeddings After: ")
                # print(tmp_h_set_emb.shape)
                # print(tmp_path_emb.shape)
                # print(tmp_t_emb.shape)

        item_emb_i = self._knowledge_attention(tmp_h_set_emb, tmp_path_emb, tmp_t_emb)
        item_embeddings.append(item_emb_i)
        ...
        tmp_h_set_emb = self.entity_emb(user_potential_triple_set[0][0])
        tmp_path_emb = self.relation_emb(user_potential_triple_set[1][0])
        tmp_t_emb = self.entity_emb(user_potential_triple_set[2][0])
        ...
        for i in range(self.n_layer):
            ...
            for k in range(1, i + 1):
                ...
                path_emb = torch.mul(
                    path_emb, self.relation_emb(user_potential_triple_set[1][i])
                )
            ...
            if i != 0:
                # print("user_potential_embeddings Before: ")
                # print(tmp_h_set_emb.shape)
                # print(tmp_path_emb.shape)
                # print(tmp_t_emb.shape)
                tmp_h_set_emb = torch.cat((tmp_h_set_emb, h_set_emb), dim=-1)
                tmp_path_emb = torch.cat((tmp_path_emb, path_emb), dim=-1)
                tmp_t_emb = torch.cat((tmp_t_emb, t_emb), dim=-1)
                # print("user_potential_embeddings After: ")
                # print(tmp_h_set_emb.shape)
                # print(tmp_path_emb.shape)
                # print(tmp_t_emb.shape)

        user_potential_emb_i = self._knowledge_attention(
            tmp_h_set_emb, tmp_path_emb, tmp_t_emb
        )
        user_potential_embeddings.append(user_potential_emb_i)
        ...
        tmp_h_set_emb = self.entity_emb(item_origin_triple_set[0][0])
        tmp_path_emb = self.relation_emb(item_origin_triple_set[1][0])
        tmp_t_emb = self.entity_emb(item_origin_triple_set[2][0])
        ...
        for i in range(self.n_layer):
            ...
            if i != 0:
                # print("item_origin_embeddings Before: ")
                # print(tmp_h_set_emb.shape)
                # print(tmp_path_emb.shape)
                # print(tmp_t_emb.shape)
                tmp_h_set_emb = torch.cat((tmp_h_set_emb, h_set_emb), dim=-1)
                tmp_path_emb = torch.cat((tmp_path_emb, path_emb), dim=-1)
                tmp_t_emb = torch.cat((tmp_t_emb, t_emb), dim=-1)
                # print("item_origin_embeddings After: ")
                # print(tmp_h_set_emb.shape)
                # print(tmp_path_emb.shape)
                # print(tmp_t_emb.shape)

        item_origin_emb_i = self._knowledge_attention(
            tmp_h_set_emb, tmp_path_emb, tmp_t_emb
        )
        item_origin_embeddings.append(item_origin_emb_i)
    ...
    def predict(
        ...
        if self.agg == "concat":
            if len(user_embeddings) != len(item_embeddings):
                raise Exception(
                    "Concat aggregator needs same length for user and item embedding"
                )
            # 对比学习的知识表示学习
            for i in range(0, len(user_embeddings)):
                kge_loss += self.RKG_Embedding_Loss(
                    user_embeddings[i],
                    user_potential_embeddings[i],
                    user_embeddings,
                    user_potential_embeddings,
                )
                kge_loss += self.RKG_Embedding_Loss(
                    item_embeddings[i],
                    item_origin_embeddings[i],
                    item_embeddings,
                    item_origin_embeddings,
                )
    ...
    def _knowledge_attention(self, h_set_emb, path_emb, t_emb):
        ...
        att_weights = self.attention(torch.cat((h_set_emb, path_emb), dim=-1)).squeeze(
            -1
        )
    ...
    def RKG_Embedding_Loss(
        self,
        current_user_inital_embedding,
        current_user_potential_embedding,
        user_inital_embedding,
        user_potential_embeddings,
    ):
        previous_user_potential_embeddings_all = user_potential_embeddings[
            0
        ]  # 用户潜在嵌入的第1层
        previous_user_inital_embedding_all = user_inital_embedding[
            0
        ]  # 用户初始嵌入的第1层
        # print(
        #     "previous_user_potential_embeddings_all shape: ",
        #     previous_user_potential_embeddings_all.shape,
        # )
        # print(
        #     "previous_user_inital_embedding_all shape: ",
        #     previous_user_inital_embedding_all.shape,
        # )
        # print("user_potential_embeddings shape: ", len(user_potential_embeddings))
        # print("user_inital_embedding shape: ", len(user_inital_embedding))
        ...
        for i in range(1, len(user_potential_embeddings)):
            ...
            # )
            # print(
            #     "user_potential_embeddings shape: ",
            #     user_potential_embeddings[i].shape,
            # )
            previous_user_potential_embeddings_all = torch.cat(
                (user_potential_embeddings[i], previous_user_potential_embeddings_all),
                dim=0,
            )
        ...
        for i in range(1, len(user_inital_embedding)):
            ...
            # )
            # print(
            #     "user_inital_embedding shape: ",
            #     user_inital_embedding[i].shape,
            # )
            previous_user_inital_embedding_all = torch.cat(
                (user_inital_embedding[i], previous_user_inital_embedding_all), dim=0
            )
        ...
        # 内积
        pos_score_user_inital = torch.mul(norm_user_inital, norm_user_potential).sum(
            dim=1
        )
        # 计算得分
        ttl_score_user_inital = torch.matmul(
            norm_user_inital, norm_all_user_inital.transpose(0, 1)
        )
        # 通过softmax函数计算得分
        pos_score_user_inital = torch.exp(pos_score_user_inital / self.ssl_temp)
        ttl_score_user_inital = torch.exp(ttl_score_user_inital / self.ssl_temp).sum(
            dim=1
        )
        # 计算损失函数
        kge_loss_user_inital = -torch.log(
            pos_score_user_inital / ttl_score_user_inital
        ).sum()
        # for item

        # for user_inital_embedding
        pos_score_user_potential = torch.mul(norm_user_potential, norm_user_inital).sum(
            dim=1
        )
        ttl_score_user_potential = torch.matmul(
            norm_user_potential, norm_all_user_potential.transpose(0, 1)
        )
        pos_score_user_potential = torch.exp(pos_score_user_potential / self.ssl_temp)
        ttl_score_user_potential = torch.exp(
            ttl_score_user_potential / self.ssl_temp
        ).sum(dim=1)
        kge_loss_user_potential = -torch.log(
            pos_score_user_potential / ttl_score_user_potential
        ).sum()
